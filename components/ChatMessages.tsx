  ...
  return (
    <div className="p-5">
      {!loading && messages?.length === 0 && (
        <div className="flex flex-col justify-center items-center text-center p-20 rounded-xl space-y-2 bg-indigo-400 text-white font-extrabold">
          <MessageCircleIcon className="h-10 w-10" />
          <h2>
            <span className="font-bold">Invite a friend</span>
            <span className="font-bold">Send your first message</span>
          </h2>
            ...
            <div
              className={`flex flex-col relative space-y-2 p-4 w-fit line-clamp-1 mx-2 rounded-lg ${
                isSender
                  ...
                  : 'bg-gray100 dark:text-gray-100 dark:bg-slate-700 rounded-bl-none'
              }`}
            >
              <p
                className={`text-sm italic font-extralight line-clamp-1 ${
                  isSender ? 'text-right' : 'text-left'
                }`}
              >
                {message.user.name.split(' ')[0]}
                {/* {!message.translated?.[language] && <LoaderSpinner/>} */}
              </p>
